...
WHERE  svr_status ='running' --and SERVERNAME like '%NETSLSQLPRE01A%'
...
SET @BODY1=  '<font size=2 color=#C35817  face=''verdana''><B>Followed by alert less than 5000MB free space:</b> </font>
...
<Table border=0 width=500 bgcolor=#ECE5B6 cellpadding=1  style="color:#7E2217;font-face:verdana;font-size:12px;">  
 ...
 <td width=150 color=white>SPACE_MB</td>  </b>  
...
--+case when @Percentage< 10 then '<td align=center style="color:#FF0000;font-weight:bold">'+ISNULL(@Percentage,'&nbsp')+'</td>'
--else '<td align=center >'+ISNULL(@Percentage,'&nbsp')+'</td>' end
...
--SELECT @EMAILIDS1= 'OryxSQLDBA@royallondon.com'
--SELECT @EMAILIDS1= 'OryxSQLDBA@royallondon.com'

SELECT @EMAILIDS1= 'abcd.com'

EXEC MSDB.DBO.SP_SEND_DBMAIL @RECIPIENTS=@EMAILIDS,
@SUBJECT = 'DBA: DISK SPACE INFO WITHOUT PERCENTAGE',
@BODY = @BODY1,
