...
/*
 ...
 in (
'\\hug\dfsroot\NIIT Support\Backups\',
'\\kw3l1p120\sqlbackups\',
'\\kw3l1p34\h$\restore\'
)

select filer_name,filer_satus,access_from_local from Backup_filers_name

*/
alter  PROCEDURE [dbo].[USP_DBA_GET_FILER_SPACE]
/*
Summary:     FILER Space Utilization findings
...
--select @fname,ltrim(output),getdate() from #Backup_filer where ltrim(output) like '%bytes free%'
...
update Backup_filers_name set filer_Satus ='no accessed' where filer_name 
in (
'\\hug\dfsroot\NIIT Support\Backups\',
'\\kw3l1p120\sqlbackups\',
'\\kw3l1p34\h$\restore\')
...
<Table border=0 width=450 bgcolor=#ECE5B6 cellpadding=1  style="color:#7E2217;font-face:verdana;font-size:12px;">  
 ...
 <td width=150 color=white>Filer Name </td> 
 <td width=150 color=white>Free Space GB</td>  
</b>  
</tr>'
...
This is an auto generated mail by Muthu. If you receive this email by mistake please contact us. 
 </br>
...
SELECT @EMAILIDS=
COALESCE(@EMAILIDS+';','')+EMAIL_ADDRESS  FROM CNA_DB.DBO.DBA_ALL_OPERATORS
WHERE STATUS =1


DECLARE @EMAILIDS1 VARCHAR(500)
SELECT @EMAILIDS1=
COALESCE(@EMAILIDS1+';','')+EMAIL_ADDRESS  FROM CNA_DB.DBO.DBA_ALL_OPERATORS
WHERE STATUS =1 and Mail_copy='CC'


EXEC MSDB.DBO.SP_SEND_DBMAIL @RECIPIENTS=@EMAILIDS,
@SUBJECT = 'DBA: Filler Share Low Space Usage',
@BODY = @BODY1,
