...
:: UPDATE 3.30.2012  Added error logging to help troubleshoot databases backup errors.   --log-error="E:\MYSQL_Backup\backupfiles\dumperrors.txt"
...
set dbpass=Il0ve$ql@321

:: Error log path - Important in debugging your issues
set errorLogPath="C:\MYSQL_Backup_New\backupfiles\dumperrors.txt"

:: MySQL EXE Path
set mysqldumpexe="C:\Program Files\MySQL\MySQL Server 8.0\bin\mysqldump.exe"

:: Error log path
set backupfldr=C:\MYSQL_Backup_New\backupfiles\

:: Path to data folder which may differ from install dir
set datafldr="C:\ProgramData_MYSQL\MySQL\MySQL Server 8.0\Data"
...
SET %%F=!%%F:@002d=-!
%mysqldumpexe% --user=%dbuser% --password=%dbpass% --port 2109 --databases --routines --log-error=%errorLogPath% %%F > "%backupfldr%%%F.%backuptime%.sql"
) ELSE (
