...
use DBAUtil
go
-- Exec USP_Repl_Row_Count_cmd_SSRS_BI @Tbl_rowcount_diff =1
alter proc USP_Repl_Row_Count_cmd_SSRS_BI
@Tbl_rowcount_diff int
as
begin
...
IF EXISTS(
select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_Repl_Row_Count_Email_Alert where rowcount_diff >@Tbl_rowcount_diff
and  (rowcount_diff<>'16' and destination_objectname in ('dbo.Home Dynamix, LLC_$Sales Line'))

/*
select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_Repl_Row_Count_Email_Alert where rowcount_diff >1
and  (rowcount_diff<>'166' and destination_objectname in ('dbo.Home Dynamix, LLC_$Sales Line'))

select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_Repl_Row_Count_Email_Alert where
rowcount_diff<>'16' and destination_objectname in ('dbo.Home Dynamix, LLC_$Sales Line')
...
EXEC spQueryToHtmlTable @html = @html OUTPUT,  
 @query = N'select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_Repl_Row_Count_Email_Alert', @orderBy = N'ORDER BY [rowcount_diff]';
...
--@copy_recipients='aa@abc.com',
    @subject = 'Replication Row Count Difference:',
    @body = @html,
    @body_format = 'HTML',
    @query_no_truncate = 1,
