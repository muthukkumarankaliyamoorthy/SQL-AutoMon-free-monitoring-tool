...
Set NOCOUNT ON;
GO
-- Exec USP_Repl_Undistriuted_cmd_SSRS_BI @pending_commands=500
...
IF EXISTS(
select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_temp_sub2_UnDist_CMD where pending_commands>= @pending_commands and publication not in ('HDXDB-DR')
-- select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_temp_sub2_UnDist_CMD where pending_commands>= 500 and publication not in ('HDXDB-DR')
...
EXEC spQueryToHtmlTable @html = @html OUTPUT,  
 @query = N'select * from [REPL_HOMESQL01\HOMESQL01].DBAUtil.dbo.TBL_temp_sub2_UnDist_CMD where publication not in (''HDXDB-DR'')', @orderBy = N'ORDER BY [pending_commands]';
