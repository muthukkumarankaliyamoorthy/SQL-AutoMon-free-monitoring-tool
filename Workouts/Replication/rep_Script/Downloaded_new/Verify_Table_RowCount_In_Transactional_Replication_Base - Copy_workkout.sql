...
/*
IF OBJECT_ID('DBAUtil..tempTransReplication') IS NOT NULL 
...
SELECT 'HDXDB' AS publisher_db, s.name AS source_owner, o.name AS source_object, o.Type_Desc AS object_type, i.rowcnt AS rowcount_publisher 
 FROM HDXDB.sys.objects AS o 
 INNER JOIN HDXDB.sys.schemas AS s 
on o.schema_id = s.schema_id 
 LEFT OUTER JOIN HDXDB.dbo.sysindexes AS i 
on o.object_id = i.id 
 WHERE 'HDXDB'COLLATE DATABASE_DEFAULT + '.' + s.name + '.' + o.name 
...
insert infrom to DBAUtil.dbo.TBL_Repl_Row_Count_Email_Alert
