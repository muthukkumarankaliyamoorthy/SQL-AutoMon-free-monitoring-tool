...
IF EXISTS(
select * from  [DBAUtil].[dbo].tbl_Replication_lag where [LAG DD:HH:MM] > '0:0:15'
)  
...
EXEC spQueryToHtmlTable @html = @html OUTPUT,  
 @query = N'select * from  [DBAUtil].[dbo].tbl_Replication_lag where [LAG DD:HH:MM] > ''0:0:15'' ', @orderBy = N'ORDER BY [LAG DD:HH:MM]';
