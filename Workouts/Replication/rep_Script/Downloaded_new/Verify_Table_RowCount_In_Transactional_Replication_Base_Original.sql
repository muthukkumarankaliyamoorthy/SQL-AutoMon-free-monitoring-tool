...
on o.object_id = i.id 
 WHERE ' + '''' + @pub_db + '''COLLATE DATABASE_DEFAULT' + ' + ' + '''' + '.' + '''' + ' + s.name' + ' + ' + '''' + '.' + '''' + ' + o.name' + ' 
...
WHILE @@FETCH_STATUS = 0 
 BEGIN
SET @strSQL_S = 'SELECT ' + '''' + @sub_srv + '''' + ' AS subscriber_srv, ' + '''' + @sub_db + '''' + ' AS subscriber_db, ' + 's.name AS destination_owner, o.name AS destination_object, o.Type_Desc AS object_type, i.rowcnt AS rowcount_subscriber 
FROM [' + @sub_srv + '].[' + @sub_db + '].sys.objects AS o 
INNER JOIN [' + @sub_srv + '].[' + @sub_db + '].sys.schemas AS s on o.schema_id = s.schema_id
LEFT OUTER JOIN [' + @sub_srv + '].[' + @sub_db + '].dbo.sysindexes AS i on o.object_id = i.id
